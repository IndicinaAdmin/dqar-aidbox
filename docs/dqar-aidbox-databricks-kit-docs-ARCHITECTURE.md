# cdar-aidbox-databricks-kit Architecture
...
```
cdar-contracts          shared interface package (versioned dependency)
        │
        ├──────────────┬───────────────────────────┐
        ▼              ▼                           ▼
cdar-client-kit   cdar-aidbox-databricks-kit   sql-on-fhir-libraries
...
```
 ...
 (per-resource, write-time)      (per-batch, → OpenMetadata)      (per-table, → Databricks)
   ...
   ol-run-id  ──────────────────▶  CDARIngestFacet.fieldMappings     dqar_source_system_id
...
The three must agree: EXT 7 `ol-run-id` == RunEvent `runId`; `CDARIngestFacet.sourceFeedId` == UC `dqar_source_feed_id`. Drift between any pair is a finding.
...
4. Orchestrator: complete_run(run_id, outputs + CDARIngestFacet(fieldMappings))
...
|---|---|---|
| `OpenLineageEmitter` | START / COMPLETE / FAIL RunEvents to OpenMetadata; builds `CDARIngestFacet` | `OPENLINEAGE_EMISSION.md` |
...
The `CDARIngestFacet` mirrors `sourceFeedId`/`sourceSystemId` into the lineage graph so the graph is self-describing, but the authoritative per-table attribution is the UC property. Two views, one truth, kept consistent.

---

## Integration Points

### From `cdar-client-kit`
- Consumes `uc-properties.json` → loaded to Databricks
- Shares types via `cdar-contracts` (the versioned interface package)

### To OpenMetadata
- Emits OpenLineage RunEvents with `CDARIngestFacet`
...
|---|---|---|
| `cdar-contracts>=1.0.0,<2.0.0` | Shared interface types | No |
